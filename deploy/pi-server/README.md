...
- **Atualizar config/shaderpacks/resourcepacks/options.txt/etc:** solte a pasta/arquivo direto em `\\microlab\modpacks\<Perfil>\` (irmã de `mods\`, ex: `\\microlab\modpacks\<Perfil>\config\`, `...\shaderpacks\`, `...\options.txt`). O watcher empacota tudo que não for `mods\`/`instance.json` num `extras.zip` e publica sozinho; o launcher sincroniza isso como *overlay* na pasta da instância do player, sem apagar saves/logs. Não solte a pasta `saves\`/mundo do seu próprio teste aqui — ela é ignorada de propósito (não deve ir pro player).
